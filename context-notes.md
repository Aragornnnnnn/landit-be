...
## 2026-08-28 LAN-358 추가 대표 예문 이미지 URL 반영

- 기존 PR #126은 V63에서 ID 1~981을 반영하므로 변경하지 않는다.
- V68이 V63보다 먼저 배포되면 Flyway가 나중에 들어온 낮은 버전을 건너뛸 수 있으므로, 신규 PR은 `feat/LAN-358`을 base로 하는 stacked PR로 올린다.
- PR #126을 먼저 병합한 뒤 신규 PR의 base를 `develop`로 변경해 병합한다.
- 열린 BE PR들의 예약 버전 V59~V67 다음인 PostgreSQL V68에서 ID 982~1938만 추가 반영한다.
- 최종 WebP는 ID 1~1938 모두 1448x1086이며, 신규 범위 957개는 S3 메타데이터와 CloudFront 표본 SHA-256 검증을 통과했다.
- 대상 ID가 하나라도 없으면 실패하고, 실제 URL과 다른 행만 갱신한 뒤 적용 결과를 다시 검증한다.
