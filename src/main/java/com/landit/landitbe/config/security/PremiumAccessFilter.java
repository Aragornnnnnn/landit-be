 ...
 * 유료 기능 API 요청을 구독 상태로 제한한다.
 *
 * <p>인증 필터 뒤에서 동작하며, 프리미엄 전용 경로만 검사한다. 경로 비교는 시큐리티 설정의 {@code requestMatchers}와 같은 {@link
 ...
 * </ul>
 *
 * <p>시나리오 대화(세션 시작, 메시지 전송, 속마음 조회, 이전 시나리오 다시 대화)는 구독과 관계없이 제한하지 않는다. 무료 사용자의 상세 피드백 잠금은 {@code
 * SessionFeedbackService}가 피드백 응답에서 판단한다. 피드백·레벨 평가 조회, 세션 종료, 표현 목록·프리톡 주제·지난 프리톡 조회,
 * 마이페이지·스트릭·메일함도 제한하지 않는다.
 */
@Component
public class PremiumAccessFilter extends OncePerRequestFilter {
  ...
  /**
   * 구독 상태 평가 Service, 접근 거부 응답 작성기, 저장된 학습 권한 Service를 주입받는다.
   *
   * @param userSubscriptionService 구독 상태 평가 Service
   * @param failureResponseWriter 접근 거부 응답 작성기
   * @param grants 이미 시작한 학습의 완료 권한을 확인하는 Service
   */
