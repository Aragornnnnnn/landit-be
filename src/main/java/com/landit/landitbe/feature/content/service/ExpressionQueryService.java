...
public class ExpressionQueryService {
  ...
  // 자산이 아직 없거나 TTS 미완성이면 빈 값 — 표현 981개의 자산을 단계적으로 채우는 동안
  // 학습 시작 화면이 깨지지 않게 하기 위한 의도된 동작이다 (앱은 URL이 null이면 해당 파트를 숨긴다).
  // 표현 TTS(expressionAudioUrl)는 완성된 자산이라도 패턴형 표현(발화 불가)이면 null이다.
