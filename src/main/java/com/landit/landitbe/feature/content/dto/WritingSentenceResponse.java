 ...
 * @param writingSentenceWords 정답 예문을 단어 단위로 나눈 배열(정답 순서 유지)
 * @param writingSentenceWordChoices 정답 단어와 오답 단어를 섞은 선택지 배열(저장된 섞인 순서 그대로)
 */
...
public record WritingSentenceResponse(
    ...
    @Schema(description = "선택된 연습 질문의 해석", example = "뮤지컬 어땠어?") String writingQuestionTranslation,
    @Schema(
  ...
  /**
   * 파싱된 추가 예문을 작문 문제 응답으로 변환한다.
   *
   * @param parsed 작문 문제로 변환할 파싱된 추가 예문
   * @return 작문 연습 문제 응답
   */
  public static WritingSentenceResponse from(ParsedPracticeSentence parsed) {
    ...
    return new WritingSentenceResponse(
        ...
        parsed.sentenceWords(),
        parsed.sentenceWordChoices());
  }
}
