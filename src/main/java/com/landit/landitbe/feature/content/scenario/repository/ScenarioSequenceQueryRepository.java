...
import com.landit.landitbe.feature.content.scenario.repository.projection.ScenarioThumbnailProjection;
...
public interface ScenarioSequenceQueryRepository extends JpaRepository<Scenario, Long> {
  ...
  /**
   * 사용자 학습 언어에 맞는 시나리오 ID를 전체 노출 순서(display_order) 오름차순으로 조회한다.
   *
   * @param userId 사용자 ID
   * @return 전체 노출 순서 오름차순으로 정렬한 시나리오 ID 목록
   */
  @Query(
      """
            SELECT s.id
  ...
  @Query(
      """
            SELECT new com.landit.landitbe.feature.content.scenario.repository.projection.ScenarioThumbnailProjection(
                s.id,
                s.thumbnailUrl
            )
            FROM Scenario s
