...
public class AdminPushCampaignService {
  ...
  private final com.landit.landitbe.feature.notification.campaign.messaging.AdminPushScheduler
      scheduler;
  ...
  public void prepareForProcessing(UUID id) {
    ...
    if (scheduled && campaign.scheduledAt().isAfter(Instant.now())) {
      throw new com.landit.landitbe.feature.notification.delivery.client
          .RetryablePushNotificationException("예약 시각 전에는 발송할 수 없습니다.");
    }
