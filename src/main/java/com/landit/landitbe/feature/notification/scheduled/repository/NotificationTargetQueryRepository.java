    ...
    jdbcTemplate.query(
        ...
        join writing_expression we on we.scenario_id = s.id
          ...
          and we.difficulty_level between
              case when coalesce(up.learning_level, 3) = 1 then 1
                   when coalesce(up.learning_level, 3) <= 3 then 2 else 4 end
              and case when coalesce(up.learning_level, 3) = 1 then 1
                       when coalesce(up.learning_level, 3) <= 3 then 3 else 5 end
