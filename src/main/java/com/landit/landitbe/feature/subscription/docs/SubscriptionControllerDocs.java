...
public interface SubscriptionControllerDocs {
   ...
   * @return 구독 상태, 프리미엄 적용 여부, 유료 구독 도입 여부, 결제 기간 종류, 만료 시각, 도입 이후 대화 완료 여부, 상품 ID, 스토어
   */
  @Operation(
      summary = "사용자 구독 상태 조회",
      description =
          "RevenueCat 웹훅으로 갱신된 서버 기준 구독 상태를 조회합니다. premium이 true면 프리미엄 혜택이 적용 중이고,"
              + " subscriptionLaunched가 false면 구독에 따른 이용 제한과 페이월을 적용하지 않습니다. true일 때는 프리미엄 전용 기능에"
              + " premium이 필요하고, 비구독자의 시나리오 대화는 conversationCompletedSinceLaunch가 false일 때 허용합니다."
