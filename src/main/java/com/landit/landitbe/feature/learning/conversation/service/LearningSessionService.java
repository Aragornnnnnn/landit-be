// 공통 학습 세션의 조회와 상태 변경을 소유한다.

package com.landit.landitbe.feature.learning.conversation.service;
 ...
 * 공통 학습 세션의 조회와 상태 변경을 소유한다.
 *
 * <p>ID로 상태를 변경하는 메서드는 호출자가 소유권과 업무 조건을 확인한 뒤 같은 트랜잭션에서 사용한다. 잠금 순서는 호출 업무가 결정하고, 반환된 조회 값은 이후
 * Entity 변경에 따라 자동 갱신되지 않는다.
 */
...
public class LearningSessionService {
   ...
   */
  public List<LearningSessionSnapshot> findSessions(List<Long> sessionIds) {
  ...
  public Optional<LearningSessionAccess> findOwnedIfPresent(long userId, long sessionId) {
    return learningSessionRepository
        .findByIdAndUserProfileId(sessionId, userId)
        .map(
            session ->
  ...
  public Optional<LocalDateTime> findOwnedStart(long userId, long sessionId, String kind) {
  ...
  /**
   * 중단된 시나리오를 같은 트랜잭션에서 진행 상태로 되돌린다.
   *
   ...
   *
   * @param sessionId 세션 ID
   * @return 세션이 존재하면 현재 값, 없으면 빈 Optional
   */
   ...
   * @param sessionId 세션 ID
   * @param userId 소유자 ID
   * @return 소유한 세션의 현재 값, 일치하는 세션이 없으면 빈 Optional
   */
  ...
  /**
   * 호출 트랜잭션 동안 소유 세션을 잠그고 현재 값을 제공한다.
   *
   * @param sessionId 세션 ID
   * @param userId 소유자 ID
   * @return 잠근 세션의 현재 값, 일치하는 세션이 없으면 빈 Optional
   */
