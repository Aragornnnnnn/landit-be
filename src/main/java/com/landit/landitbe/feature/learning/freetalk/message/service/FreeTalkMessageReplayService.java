...
public class FreeTalkMessageReplayService {
  ...
  public FreeTalkMessageSubmitResponse findCompletedResponse(
    ...
    SessionHistoryMessageSnapshot userMessage =
        conversationMessageService
            .findByClientMessage(history.getId(), request.clientMessageId())
            .orElse(null);
    if (userMessage == null) {
      return null;
    }
  ...
  private SessionHistoryMessageSnapshot requireNextAiMessage(
      List<SessionHistoryMessageSnapshot> messages, int userMessageIndex) {
    if (userMessageIndex + 1 >= messages.size()) {
      throw new ApiException(ErrorCode.CONFLICT);
    }
    SessionHistoryMessageSnapshot nextMessage = messages.get(userMessageIndex + 1);
  ...
  private int indexOfMessage(List<SessionHistoryMessageSnapshot> messages, long messageId) {
  ...
  private long speakingDurationUntil(
      List<SessionHistoryMessageSnapshot> messages, int messageSequence) {
    return messages.stream()
        .filter(message -> message.getRole() == ConversationSpeaker.USER)
        .filter(message -> message.getMessageSequence() <= messageSequence)
        .map(SessionHistoryMessageSnapshot::getUtteranceDurationMs)
