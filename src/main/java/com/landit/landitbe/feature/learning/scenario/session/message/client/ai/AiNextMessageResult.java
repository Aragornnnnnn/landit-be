// AI 다음 메시지 생성 결과를 표현한다.

package com.landit.landitbe.feature.learning.scenario.session.message.client.ai;

import com.landit.landitbe.feature.learning.scenario.session.domain.GoalCompletionStatus;

/**
 * AI 다음 메시지 생성 결과를 표현한다.
 *
 * @param acknowledgement AI가 생성한 맞장구
 * @param translatedAcknowledgement 번역된 맞장구
 * @param goalCompletionStatus 대화 목표 달성 상태
 */
public record AiNextMessageResult(
    String acknowledgement,
    String translatedAcknowledgement,
    GoalCompletionStatus goalCompletionStatus) {}
