...
public interface ScenarioSessionStartQueryRepository extends JpaRepository<Scenario, Long> {
  ...
  @Query(
      """
            SELECT new com.landit.landitbe.feature.session.infrastructure.ScenarioSessionStartRow(
                s.id,
                s.categoryId,
                c.status,
                s.status,
  ...
  @Query(
      """
            SELECT new com.landit.landitbe.feature.session.infrastructure.ScenarioSessionLockRow(
                previousScenario.id,
                usp.status
            )
            FROM UserProfile up
