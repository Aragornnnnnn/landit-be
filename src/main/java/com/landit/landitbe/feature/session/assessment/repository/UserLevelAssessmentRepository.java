...
package com.landit.landitbe.feature.session.assessment.repository;
...
public interface UserLevelAssessmentRepository extends JpaRepository<UserLevelAssessment, Long> {
   ...
   * @param launchedAt 서비스 시간대의 결제 도입 시각
  ...
  @Query(
      value =
          """
          SELECT EXISTS (
              SELECT 1 FROM user_level_assessment a
              JOIN learning_session s ON s.id = a.learning_session_id
              WHERE a.user_profile_id = :userProfileId AND source = 'MODEL'
                AND s.ended_at >= :launchedAt
          ...
          """,
      nativeQuery = true)
  boolean existsInitializedLevelSince(
      @Param("userProfileId") Long userProfileId, @Param("launchedAt") LocalDateTime launchedAt);
}
