...
class NpsApiIntegrationTests {
    ...
    void submitNpsStoresAuthenticatedUsersScoreAndOpinion() throws Exception {
        ...
        mockMvc.perform(npsRequest(accessToken, """
                {
                  "score": 3,
    ...
    void submitNpsRejectsMissingOrOutOfRangeScore() throws Exception {
        String accessToken = login("nps-invalid", "nps-invalid@example.com", "Invalid User", "nps-invalid-nonce");

        for (String request : List.of("{}", "{\"score\":0}", "{\"score\":6}")) {
            mockMvc.perform(npsRequest(accessToken, request))
    ...
    void submitNpsRejectsRequestWithoutAccessToken() throws Exception {
        mockMvc.perform(post("/api/v1/nps")
    ...
    /** NPS 제출 요청을 만든다. */
    private MockHttpServletRequestBuilder npsRequest(
            String accessToken,
            String content
    ) {
    ...
    private void submit(String accessToken, String content) throws Exception {
        mockMvc.perform(npsRequest(accessToken, content))
                .andExpect(status().isCreated());
    }
