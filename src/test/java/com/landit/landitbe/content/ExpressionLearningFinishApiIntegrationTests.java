...
class ExpressionLearningFinishApiIntegrationTests {
    ...
    void learningFinishRejectsMissingAccessToken() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/expressions/{expressionId}/learning-finish", expressionId))
    ...
    void learningFinishCreatesCompletionForUnlockedExpression() throws Exception {
        ...
        String accessToken = login("google-finish-1", "finish@example.com", "Finish User", "finish-nonce");
        Long userProfileId = findUserProfileIdByEmail("finish@example.com");

        // when: 해금된 1번 표현을 완료하면
        // then: 200 + data는 빈 객체({}), error는 null
        mockMvc.perform(post("/api/v1/expressions/{expressionId}/learning-finish", firstExpressionId)
    ...
    /**
     * 이미 완료한 표현을 다시 완료하면 새 기록 없이(1건 유지),
     * 최초 완료 시각(completed_at)은 보존하고 마지막 완료 시각(last_completed_at)만 갱신하는지 검증한다.
     */
    @Test
    void learningFinishRenewsLastCompletedAtForAlreadyCompletedExpression() throws Exception {
        ...
        Long scenarioId = seedScenarioWithExpressions();
        Long firstExpressionId = findExpressionIdByDisplayOrder(scenarioId, 1);
        String accessToken = login("google-finish-2", "finish2@example.com", "Finish User2", "finish-nonce-2");
    ...
    void learningFinishRejectsLockedExpression() throws Exception {
        ...
        String accessToken = login("google-finish-3", "finish3@example.com", "Finish User3", "finish-nonce-3");
        Long userProfileId = findUserProfileIdByEmail("finish3@example.com");

        // when: 잠긴 3번 표현을 완료하려 하면
        // then: 403 + EXPRESSION_LOCKED
        mockMvc.perform(post("/api/v1/expressions/{expressionId}/learning-finish", thirdExpressionId)
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
    ...
    void learningFinishRejectsUnknownExpression() throws Exception {
        ...
        String accessToken = login("google-finish-4", "finish4@example.com", "Finish User4", "finish-nonce-4");

        // when: DB에 없는 ID(999999)로 완료하려 하면
        // then: 404 + RESOURCE_NOT_FOUND
        mockMvc.perform(post("/api/v1/expressions/{expressionId}/learning-finish", 999_999L)
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
    ...
    /** 시나리오 1개 + EN/KR 표현 5개(displayOrder 1~5)를 심고 시나리오 PK를 반환한다. (부모 FK 때문에 category → scenario → writing_expression 순) */
    ...
    private void insertWritingExpression(Long scenarioId, int displayOrder, LocalDateTime now) {
        jdbcTemplate.update(
                ...
                scenarioId, displayOrder, "expression-" + displayOrder, "표현-" + displayOrder, "[]", now, now
        );
    }
    ...
    private void insertCompletion(
            ...
            LocalDateTime completedAt, LocalDateTime lastCompletedAt
    ) {
        jdbcTemplate.update(
                "INSERT INTO user_writing_expression_completion "
                        ...
                        + "VALUES (?, ?, ?, ?, ?)",
                userProfileId, scenarioId, writingExpressionId, completedAt, lastCompletedAt
        );
    }
    ...
    private int countCompletions(Long userProfileId, Long writingExpressionId) {
        return jdbcTemplate.queryForObject(
                ...
                Integer.class, userProfileId, writingExpressionId
        );
    }
    ...
    private LocalDateTime queryTime(Long userProfileId, Long writingExpressionId, String column) {
        return jdbcTemplate.queryForObject(
                "SELECT " + column + " FROM user_writing_expression_completion "
                        + "WHERE user_profile_id = ? AND writing_expression_id = ?",
                LocalDateTime.class, userProfileId, writingExpressionId
        );
    }
    ...
    private Long insertAndGetId(String sql, Object... args) {
        ...
        jdbcTemplate.update(connection -> {
            ...
            for (int i = 0; i < args.length; i++) {
                statement.setObject(i + 1, args[i]); // statement = DB에 보낼 SQL 명령서 (SQL + 파라미터를 모두 가지고 있는 객체)
            }
            return statement;
        }, keyHolder);
