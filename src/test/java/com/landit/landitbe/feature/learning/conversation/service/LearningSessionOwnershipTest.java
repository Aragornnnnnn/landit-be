...
class LearningSessionOwnershipTest {
        ...
        .thenReturn(
            Optional.of(
                new LearningSessionAccess(
                    com.landit.landitbe.feature.learning.conversation.domain.SessionType.FREE_TALK,
                    com.landit.landitbe.feature.learning.conversation.domain.LearningSessionStatus
                        .IN_PROGRESS,
                    NOW)));
  ...
  void verifiedStartRequiresMatchingOwnerAndSessionType() {
    ...
    var session =
        mock(com.landit.landitbe.feature.learning.conversation.domain.LearningSession.class);
    when(repository.findByIdAndUserProfileId(100L, USER_ID)).thenReturn(Optional.of(session));
    when(session.getSessionType())
        .thenReturn(com.landit.landitbe.feature.learning.conversation.domain.SessionType.SCENARIO);
