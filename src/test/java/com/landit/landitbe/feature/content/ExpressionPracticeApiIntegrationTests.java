...
class ExpressionPracticeApiIntegrationTests {
  ...
  void practiceRejectsMissingAccessToken() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/expressions/{expressionId}/practice", expressionId))
  ...
  void practiceReturnsExamplesAndWritingSentence() throws Exception {
    ...
    Long expressionId = seedExpressionWithPracticeExamples();
    String accessToken =
        login("google-practice-1", "practice@example.com", "Practice User", "practice-nonce");
  ...
  void practiceRejectsUnknownExpression() throws Exception {
    ...
    String accessToken =
        login("google-practice-2", "practice2@example.com", "Practice User2", "practice-nonce-2");

    // when: DB에 없는 ID(999999)로 호출하면
    // then: 404 + RESOURCE_NOT_FOUND
    mockMvc
        .perform(
            get("/api/v1/expressions/{expressionId}/practice", 999_999L)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
  ...
  void practiceRejectsInactiveExpression() throws Exception {
    ...
    String accessToken =
        login("google-practice-3", "practice3@example.com", "Practice User3", "practice-nonce-3");
