...
class ExpressionApiIntegrationTests {
  ...
  @Autowired private MockMvc mockMvc; // 가짜 HTTP 요청을 보내는 도구
  ...
  void getExpressionsRejectsMissingAccessToken() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/expressions/{scenarioId}", scenarioId))
  ...
  void getExpressionsUnlocksEveryExpressionWhenAllCompleted() throws Exception {
    ...
    for (int displayOrder = 1; displayOrder <= 5; displayOrder++) {
      markExpressionCompleted(
          userProfileId, scenarioId, findExpressionIdByDisplayOrder(scenarioId, displayOrder));
    }
  ...
  void getExpressionsRejectsUnknownScenario() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/expressions/{scenarioId}", 999_999L)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken))
  ...
  void getExpressionsReturnsOnlyUserLocaleExpressions() throws Exception {
    ...
    Long scenarioId = seedScenarioWithExpressions();
    insertWritingExpression(
        scenarioId, "EN", "JA", 1, "ja-only expression", "日本語訳", LocalDateTime.now());
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.data.length()").value(5))
        .andExpect(
            jsonPath("$.data[*].targetExpressionText")
                .value(
   ...
   * 심는다: category → scenario → writing_expression. 표현을 일부러 displayOrder 역순(5→1)으로 INSERT해서, API가
   * 정렬을 제대로 하는지도 함께 검증된다.
   */
  ...
  /**
   * INSERT를 실행하고 DB가 자동 생성한 PK(id)를 돌려주는 유틸. (범용 메서드) H2가 PostgreSQL의 "RETURNING id" 문법을 지원하지 않아서
  ...
  private void insertWritingExpression(
    ...
    jdbcTemplate.update(
        "INSERT INTO writing_expression "
            ...
            + "VALUES (?, 'DAILY_ROUTINE', 'BASIC', 3, ?, ?, ?, ?, ?, 'usage summary', "
  ...
  private void markExpressionCompleted(
