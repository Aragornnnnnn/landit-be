...
class PushDeliveryServiceTest {
  ...
  private static final long PUSH_DEVICE_ID = 2L;
  ...
  /** 각 테스트에서 발송 이력 Repository와 Push Device Service를 사용하는 Service를 생성한다. */
  @BeforeEach
  void setUp() {
    pushDeliveryService = new PushDeliveryService(pushDeliveryRepository, pushDeviceService);
  }

  /** 발송 직전에 설치가 비활성화되면 발송 이력을 만들지 않는다. */
  @Test
  void skipsDeviceThatIsNoLongerSendable() {
    when(pushDeviceService.findLockedSendableDeliveryTarget(PUSH_DEVICE_ID, USER_ID))
  ...
  /** Push Delivery Service는 Push Device Repository를 직접 소유하지 않는다. */
  @Test
  void doesNotOwnPushDeviceRepository() {
    assertThat(PushDeliveryService.class.getDeclaredFields())
        .extracting(java.lang.reflect.Field::getType)
        .doesNotContain(PushDeviceRepository.class);
  }

  /** DeviceNotRegistered Ticket 오류를 기록하고 해당 설치 Token을 무효화한다. */
  @Test
  void invalidatesDeviceForRejectedTicket() {
    ...
    verify(pushDeviceService).invalidateCurrentTokenOwner("ExponentPushToken[sent-token]");
  }
