...
- 페이지에서 선점한 기기별 발송 이력은 Expo Push 요청 최대 100건으로 묶어 직접 발송한다. 사용자별 `PUSH_SEND` 메시지를 Push Queue에 다시 넣지 않는다.
...
이번 구현은 앱 설치 상태, 세 가지 학습 알림 정책, 예약 대상 계산, Expo 전달·Receipt 추적까지 책임진다. 나머지 네 가지 제품 알림 유형과 사용자 행동에 따른 즉시 알림은 후속 작업으로 분리한다.
...
`ScheduledNotificationService`는 500명씩 대상을 계산하고, 선정된 사용자의 발송 가능한 설치를 대상으로 `push_delivery` 이력을 먼저 선점한다. `NotificationDispatchService`는 페이지에서 선점된 알림을 최대 100건씩 Expo에 보낸다. 중복 방지 키는 `push:{eventId}:{pushDeviceId}`다.
...
dev에서는 인증 사용자가 아래 API로 자기 기기에 일반 테스트 알림을 직접 요청한다.
...
이 API는 `NotificationDispatchService`를 호출해 Push Device, Expo, Ticket·Receipt 흐름을 검증한다. `landit.notification.test-api-enabled=true`일 때만 생성되며 prod에는 활성화하지 않는다.
