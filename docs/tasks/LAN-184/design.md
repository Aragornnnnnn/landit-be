...
Flyway는 `classpath:db/migration`과 `classpath:db/{vendor}`를 함께 읽는다. 최신 `origin/develop`의 전체 위치를 합치면 최고 버전은 공통 migration의 V73이므로 이번 migration은 `V74__add_push_delivery_content_variant.sql`과 `V75__add_learning_session_user_started_index.sql`로 만든다. V75는 H2와 PostgreSQL 파일로 분리하며, 운영 PostgreSQL에서는 비트랜잭션 `CREATE INDEX CONCURRENTLY`와 session-level advisory lock을 사용해 쓰기 차단을 피한다.
