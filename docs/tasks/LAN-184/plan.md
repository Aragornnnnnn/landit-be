...
| 발송 | 사용자·알림 내용이 확정된 Queue 메시지를 받아 Token별 이력을 선점하고 Expo 최대 100건씩 Ticket 발송 |
...
- 이번 PR은 UserPushToken, Queue Consumer, Expo 발송, Ticket·Receipt와 멱등성 같은 공통 전달 인프라까지만 책임진다.
...
- [ ] Expo Push Token을 등록한 dev 계정으로 테스트 API를 호출해 iOS·Android 실기기 수신을 확인한다.
