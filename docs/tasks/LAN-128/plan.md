...
- AI 시작은 질문 1번을 시작 메시지와 목록 미리보기로 사용하고, 이후 질문 2번과 3번을 조회하는지 확인했다.
- 사용자 시작은 질문 1번부터 3번까지 연속 조회한 뒤 종료되는지 확인했다.
- `./gradlew test --no-daemon`을 실행해 139개 테스트가 통과했다.
- 고정 질문 데이터와 Flyway 마이그레이션은 추가하지 않았다.
