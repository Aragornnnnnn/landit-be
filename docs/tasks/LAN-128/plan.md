...
- PostgreSQL 테스트 DB에 `./gradlew migrateDatabase`를 실행해 V18 적용을 확인한다.
...
- V18 스키마 테스트가 추가 전 실패하고, 마이그레이션 추가 후 통과하는 것을 확인했다.
- `./gradlew test --no-daemon`을 실행해 H2 전체 테스트가 통과했다.
- PostgreSQL `migrateDatabase`는 접속 환경 변수 없이 실행할 수 없어 미검증 상태다.
