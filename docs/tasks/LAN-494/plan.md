...
- 열린 BE PR 14개의 공통·DB별 마이그레이션을 확인했다. #204의 V112와 충돌하므로 이 변경은 최대 사용 번호 V124 다음인 V125를 사용한다. #204~#214의 선행 마이그레이션이 포함된 배포 이후 적용하며, 병합 전 번호와 배포 순서를 다시 확인한다.
- V125는 미완료이면서 오답이 2회 이상인 문제의 실제 두 번째 오답 시각을 복원한다. 이미 완료된 문제의 시각과 원본 제출 이력은 보존하고, 모든 문제가 종료된 복습만 마지막 문제 종료 시각으로 완료 처리한다.
...
- PostgreSQL 15에서 보정 SQL의 두 번째 오답 시각 복원, 부분 진행 유지, 첫 오답 유지, 기존 완료 시각·정오답 이력 보존 및 재실행 결과를 검증했다. V112에서 V125로 옮길 때 SQL 내용은 바꾸지 않았다. 운영 DB에는 적용하지 않았다.
