...
- 오래 학습·복습한 순서의 후보 30개 중 무작위 선정한다. 유효한 문제가 없는 표현은 제외한다.
...
- `./gradlew spotlessApply check` 통과. JUnit XML 합계 1,240건 중 실패·오류 0건, 건너뜀 9건이다. 복습 통합 테스트 11건과 메시지 라우팅 테스트를 포함한다.
