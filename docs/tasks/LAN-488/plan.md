...
## 2026-09-18~19 나머지 테스트 전체 정리

### 범위와 기준

- 시나리오 피드백 정리 이후 `e99de54c`를 기준으로 나머지 테스트와 LAN-461·491·494·300의 고유 테스트를 검토했다. 운영 소스, API, 마이그레이션, 빌드 설정은 변경하지 않는다.
- 긴 SQL·JSON·mock 준비는 의도를 드러내는 메서드로 분리하고, 응답·저장·권한·외부 호출처럼 독립된 결과는 별도 테스트로 나눴다. 동시성, 재시도, 보상 처리의 실행 순서와 핵심 기대 결과는 테스트 본문에 유지한다.
- 20줄 초과 메서드는 분리 가능성을 검토하는 기준이다. 하나의 계약을 검증하는 여러 필드나 경합 순서를 숨기기 위해 일률적으로 줄이지 않았다.

### LAN-488 변경과 검증

| 항목 | 변경 전 | 변경 후 |
| --- | --- | --- |
| 테스트 메서드 | 179개 파일, 1,169개 | 179개 파일, 1,208개 |
| 80줄 초과 테스트 | 9개 | 0개 |
| 40줄 초과 테스트 | 87개 | 63개 |
| 반복된 인증 JSON 요청 | 헤더·Content-Type·본문 설정 반복 | `AuthenticatedJsonRequests`로 설정 공유, 경로·본문·기대 응답은 호출부에서 확인 |
| 큰 검증 흐름 | 여러 결과가 한 메서드에서 실패 | 관리자 응답, 진단·평가, 프리톡, 알림, 마이그레이션의 검증 목적별 실패 위치 분리 |

- 메서드 수와 길이는 같은 JDK 구문 트리 집계 방식으로 비교했다. 이전 1,168개 집계는 완전 수식 이름으로 선언한 `@ParameterizedTest` 1개를 누락했다. 해당 테스트의 한국어 `@DisplayName`도 보완했다.
- 1,208개 테스트 메서드에 한국어 설명이 있고 파일 내 설명 중복은 없다. 테스트 분리에 따른 실행 수 증가이며 새로운 제품 동작을 추가한 것은 아니다.
- 기존 검증 호출을 구문 트리로 대조했다. 달라진 표현은 응답 파싱·사용량 조회의 공통화와 비동기 완료 대기 위치 이동이며 원래 기대 결과를 유지했다.
- 관리자 시나리오 테스트는 다른 Spring 컨텍스트의 표현 데이터와 공유하던 H2 DB를 분리했다. 표현 로그 검증은 실패 시에도 appender를 해제한다. 비동기 메시지 테스트는 차단을 해제한 뒤 완료를 기다린다.
- `./gradlew spotlessApply check` 통과: **1,304건 실행, 1,298건 성공, 실패·오류 0건, 기존 환경 조건 생략 6건**. `git diff --check` 통과.
- 생략은 PostgreSQL 전용 관리자 SQL 4건, 실제 AI 1건, 교차 저장소 1건이다. DB 적용·배포·실제 기기 동작은 검증하지 않았다.

### 후속 브랜치 반영

- 기존 원격 HEAD를 `backup/LAN-*-before-test-cleanup-20260919`로 보존했다. 이전 부모의 커밋을 경계로 `rebase --onto`해 선행 피드백 정리와 이번 테스트 정리를 함께 반영했다.
- 선행 순서는 LAN-488 → LAN-461 → LAN-491 → LAN-494이고 LAN-300은 LAN-488에서 분기한다. 각 고유 테스트, 운영 소스 보존과 전체 check를 확인했다. 게시할 때는 백업한 원격 SHA를 lease로 지정하고 게시 후 HEAD를 재확인한다.

- LAN-491의 복수 정답 예문·조회 준비를 분리했다. 리베이스 충돌은 언어별 단어 검증 메서드에 복수 정답 배열 검증을 함께 유지해 해결했다.
- LAN-494의 인증·고정 문제·오답 이동·변경된 재전송 거부·완료 검증을 분리했다. LAN-461의 동시 최초 등록과 LAN-300의 무작위 선택 테스트는 현재의 짧고 응집된 검증을 유지했다.
- 모든 브랜치에서 한국어 테스트 설명 누락과 파일 내 설명 중복이 없고 `git diff --check`를 통과했다. 이전 원격 대비 `src/main` 및 빌드 설정 차이는 없으며 V108·V109·V110·V111을 그대로 보존했다.
- 각 브랜치에서 Spotless·Checkstyle을 포함한 전체 `check`를 통과했다. LAN-491·494는 PostgreSQL 환경 전용 진입점 3건이 추가로 생략된다.

| 브랜치 | 전체 실행 | 성공 | 환경 조건 생략 |
| --- | --- | --- | --- |
| LAN-488 | 1304 | 1298 | 6 |
| LAN-461 | 1311 | 1305 | 6 |
| LAN-491 | 1328 | 1319 | 9 |
...
- 실패·오류는 모두 0건이다. 마지막 검증 기록 커밋을 후속 브랜치에 반영할 때는 검사 완료 시점과 `src` tree가 동일한지 대조한다.
