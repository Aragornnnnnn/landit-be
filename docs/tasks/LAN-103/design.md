...
- Checkstyle은 13.8.0에 포함된 `google_checks.xml`을 사용하며, 심각도를 오류로 설정한다.
- 정적 분석으로 판별할 수 없는 규칙은 기존 코드 리뷰 규칙으로 유지한다.

## 저장소 로컬 규칙

새 Java 소스 파일 첫 줄의 한국어 역할 주석은 유지한다. 이 규칙은 Google Java Style Guide의 일반 소스 파일 순서에 대한 저장소 로컬 예외다. Checkstyle의 Google 규칙 구성은 이 주석의 존재 여부를 검사하지 않으며, 별도의 주석 검사 도구도 이번 이슈 범위에 포함하지 않는다.
