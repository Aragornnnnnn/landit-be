...
- 원격 AI 호출은 연결 제한 5초와 요청 전체 제한 60초를 적용한다. 세션 최종 피드백만 생성 시간이 길 수 있어 120초를 적용한다. Java `HttpClient`에는 read timeout 대신 요청 전체 timeout을 사용한다.
...
- PR #20 리뷰에 따라 `AiClientProperties`의 연결·요청 타임아웃을 원격 AI 클라이언트에 적용하고, 지연된 세션 피드백 요청이 `FEEDBACK_GENERATION_FAILED`로 변환되는 회귀 테스트를 추가했다.
- 세션 최종 피드백이 60초를 넘길 수 있는 운영 조건을 반영해 일반 원격 AI 요청과 분리된 120초 제한을 추가했다.
