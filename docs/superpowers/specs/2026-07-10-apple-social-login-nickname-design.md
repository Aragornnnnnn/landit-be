...
- 신규 Apple 요청에 `nickname`이 없으면 기본값 `Guest`를 사용한다.
- 기존 Apple 요청에 `nickname`이 없으면 저장된 nickname을 유지한다.
...
4. 제공자가 Apple이고 요청 `nickname`이 비어 있으면 신규 사용자는 `Guest`로 생성하고, 기존 사용자의 nickname은 유지한다.

## 검증

- Apple 요청의 `nickname`이 ID Token 이름보다 우선 적용되는지 통합 테스트로 검증한다.
- Apple 요청에서 `nickname`이 없을 때 신규 사용자가 `Guest`로 생성되는지 검증한다.
- 기존 Apple 사용자가 `nickname` 없이 재로그인해도 이름이 유지되는지 검증한다.
