...
- 사용자가 이슈 번호 없이 `origin/develop` 직접 작업을 명시하면 예외로 처리하고 작업별 기록에 남깁니다.
- 작업별 기록은 `docs/tasks/{ISSUE_NUMBER}/` 아래에 둡니다.
- 작업 중 결정 사항은 `docs/tasks/{ISSUE_NUMBER}/context-notes.md`에 이어서 기록합니다.
- 여러 단계 작업은 `docs/tasks/{ISSUE_NUMBER}/checklist.md`를 갱신하며 진행합니다.
