...
- Java 포맷과 자동 검사 범위는 [Java 코드 스타일 문서](docs/development/java-style.md)를 따릅니다.
- Spotless는 Java 포맷만 자동 검사하며, 명명, Javadoc, 코드 구조와 첫 줄 한국어 역할 주석은 이 문서와 코드 리뷰로 확인합니다.
...
- 코드 변경 후 최소 검증 명령은 `./gradlew check`입니다. 이 명령은 Spotless, Checkstyle, 애플리케이션 테스트를 함께 실행합니다.
