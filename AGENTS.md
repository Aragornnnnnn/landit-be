...
- 사용자가 이슈 번호 없이 `origin/develop` 직접 작업을 명시하면 예외로 처리하고 `context-notes.md`에 남깁니다.
- 작업 중 결정 사항은 기존 `context-notes.md`에 이어서 기록합니다.
- 여러 단계 작업은 기존 `checklist.md`를 갱신하며 진행합니다.
